...
## ⚙️ Architecture & Data Workflow Diagrams

The diagram below outlines how M3U playlists and user configurations flow through the Express Server and Vite core app, directly into the player interface.

```
                  ┌───────────────────────────────────────────────┐
                  │          External IPTV Provider (M3U / EPG)   │
                  └───────────────────────┬───────────────────────┘
                                          │ URL / API Feed
                                          ▼
                  ┌───────────────────────────────────────────────┐
                  │       Node.js Express Proxy Controller        │
                  │ (Eliminates CORS on HLS / TS / MPEG Streams)  │
                  └───────────────────────┬───────────────────────┘
                                          │ Raw Streams
                                          ▼
                  ┌───────────────────────────────────────────────┐
                  │          HLS.js Stream Receiver Engine        │
                  │   - Chunk buffering & resolution adaptation   │
                  └───────────────────────┬───────────────────────┘
                                          │ Render Callback
                                          ▼
                  ┌───────────────────────────────────────────────┐
                  │        Afterglow TV HTML5 Canvas Interface     │
                  │ - Custom Video Controls                       │
                  │ - Remote Event Simulation (D-pad, Joystick)   │
                  └───────────────────────────────────────────────┘
```

---

## 📲 Guide: Building Your Own Mobile App (Custom APK Setup)

Since Afterglow TV is built on **React 19 + Vite**, it can easily be converted into an **Android Native App (APK)** using **Ionic Capacitor**. 

### Prerequisite Dependencies
To compile an APK, ensure your computer has:
1. **Node.js** (v18+)
2. **Android Studio** (specifically SDK version 33 or 34, Android SDK Command-line Tools, and Gradle)
3. **Java JDK 17** (Required for modern Android builds)

### 🚀 Step-by-Step Compilation Guide

#### 1. Integrate Ionic Capacitor
Open a terminal in the root directory and run the following commands:
```bash
# Add Capacitor Core and Android integrations
npm install @capacitor/core @capacitor/cli

# Initialize Capacitor (Set App Name to "Afterglow TV" and ID to "tv.afterglow.app")
npx cap init "Afterglow TV" tv.afterglow.app --web-dir=dist
```

#### 2. Install Native Platform Wrapper
```bash
# Install Android Platform
npm install @capacitor/android

# Add the platform container to your codebase
npx cap add android
```

#### 3. Update Build Target Paths
Open the generated `capacitor.config.json` folder in your project's root folder and verify that `webDir` is targeted to compile the Vite web app output directory:
```json
{
...
#### 4. Compile the Web Assets
Compile the React-Vite front-end source code to generate the production client directory:
```bash
npm run build
```

#### 5. Synchronize with the Native Project Workspace
Push the compiled production code into your local Android Gradle workflow folder:
```bash
npx cap sync android
```

#### 6. Open, Edit, and Build Your APK inside Android Studio
Fire up Android Studio targeting your native project wrapper:
```bash
npx cap open android
```
Once Android Studio loads, complete the compilation:
1. Wait for local **Gradle sync** to finish.
2. In the top navbar, go to **Build** ➡️ **Build Bundle(s) / APK(s)** ➡️ **Build APK(s)**.
3. Android Studio will build the package. Once completed, a notification bubble will appear with a clickable **"locate"** button showing your completed, run-ready debug APK file!
